...
router.get('/', async (_, res) => {
  try {
    ...
    const postgresCommand = `PGPASSWORD=${process.env.POSTGRES_PASSWORD} pg_dump -h ${process.env.POSTGRES_HOST} -U ${process.env.POSTGRES_USER} -d ${process.env.POSTGRES_NAME} -f ${backupFilePath}`
